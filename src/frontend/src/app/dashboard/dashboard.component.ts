...
import { Course, DS, Assignment } from '../tokens';
...
export class DashboardComponent implements OnInit {
  ...
  recent = signal<Array<{ id: number; title: string; status: string; score: number | null; max: number; req_eval?: number; created_at?: string }>>([]);
  ...
  private buildRecent() {
    const courses = this.courses() ?? [];
    const items: Array<{ title: string; status: string; score: number | null; max: number; created_at?: string }> = [];
    courses.forEach(c => {
      (c.assignments ?? []).forEach((a: any) => {
        const status = a.requiresStaffReview ? 'under_review' : (a.submissionsCount && a.submissionsCount > 0 ? 'submitted' : 'pending');
          ...
          if (!c.assignments || c.assignments.length === 0) {
            this.assignmentService.getAssignments(c.id).subscribe({
              next: (assigns) => {
                ...
                if (idx >= 0) {
                  updated[idx] = { ...updated[idx], assignments: assigns } as Course;
